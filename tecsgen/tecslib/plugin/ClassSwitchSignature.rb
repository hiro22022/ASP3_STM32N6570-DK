...
class ClassSwitchSignaturePlugin < SignaturePlugin
...
# generate 文の場合、出現次第呼び出される
  ### 意味解析段階で呼び出されるメソッド ### <<< コメント誤り (V1.4.2)
    ...
    def gen_cdl_file file
        file.print <<EOT
generate( TransparentMarshalerPlugin, #{@signature.get_namespace_path.to_s}, "");

composite t#{@signature.get_namespace_path.get_global_name}ClassSwitch {
    entry #{@signature.get_namespace_path.to_s} eEntry;
    call  #{@signature.get_namespace_path.to_s} cStraight;
    call  #{@signature.get_namespace_path.to_s} cRPC;
    cell 
};
EOT
    end
